...
connection = pika.BlockingConnection(
    pika.ConnectionParameters(
        os.environ["rmq_ip"],
        credentials=pika.PlainCredentials("admin", os.environ["rmq_passwd"]),
    )
)
...
def listen_for_requests():
    ...
    channel.basic_consume(
        queue="database_queue", on_message_callback=callback, auto_ack=False
    )
    ...
    try:
        ...
        if cursor.description is None:
            ...
            if affected_rows > 0:
                ...
                send_db_response(
                    {"status": "error", "message": "No rows affected."}, correlation_id
                )
        else:
            ...
            send_db_response(
                {"status": "success", "results": serialized_results}, correlation_id
            )
    except mysql.connector.Error as err:
        print(f"Error: {err}")
